...
class Ticket_model extends CI_Model
{
	...
	public function buscar_relaciones_ticket($todo)
	{
		...
		foreach ($todo as $key => $ticket) {
			...
			$todo['tickets'][$key]['typeRequestFor'] = @$quuery->result_array()[0];

			$this->db->select("*")->from("tb_typeticket");
			$quuery                   = $this->db->where("idTypeTicket = " , @$ticket['idTypeTicketKf'])->get();
			$todo['tickets'][$key]['typeticket'] = @$quuery->result_array()[0];
			...
			$todo['tickets'][$key]['building'] = @$quuery->result_array()[0];
			if (! is_null(@$ticket['idBuildingKf'])) {
				//print($this->checkIsDeviceOnline($ticket['idBuildingKf']));
				$todo['tickets'][$key]['building']['isHasInternetOnline']=$this->checkIsDeviceOnline(@$ticket['idBuildingKf']);
			}else{
				//print("No defined");
				$todo['tickets'][$key]['building']['isHasInternetOnline']="false";
			}
			...
			if (@$ticket['idTypeRequestFor']==1){
				...
				$todo['tickets'][$key]['department'] = @$quuery->result_array()[0];
				...
				$todo['tickets'][$key]['userMadeBy'] = @$quuery->result_array()[0];
				...
				$todo['tickets'][$key]['userRequestBy'] = @$quuery->result_array()[0];

				$this->db->select("*")->from("tb_clients");
				$quuery                    = $this->db->where("idClient = " , $todo['tickets'][$key]['building']['idClientAdminFk'])->get();
				$todo['tickets'][$key]['clientAdmin'] = @$quuery->result_array()[0];
				...
				$todo['tickets'][$key]['clientCompani'] = @$quuery->result_array()[0];
			...
			$todo['tickets'][$key]['userDelivery'] = @$quuery->result_array()[0];

			$this->db->select("*")->from("tb_ticket_delivery");
			$quuery                   = $this->db->where("id = " , @$ticket['idDeliveryTo'])->get();
			$todo['tickets'][$key]['deliveryTo'] = @$quuery->result_array()[0];
			...
			$todo['tickets'][$key]['deliveryAddress'] = @$quuery->result_array()[0];
			if (@$ticket['idTypeDeliveryKf']=='1' && $todo['tickets'][$key]['deliveryAddress']==null){
				$todo['tickets'][$key]['deliveryAddress']['address']="Carlos Calvo 3430";
				$todo['tickets'][$key]['deliveryAddress']['province']="Ciudad Autonoma de Buenos Aires";
				$todo['tickets'][$key]['deliveryAddress']['location']="Boedo";
			}
			...
			$todo['tickets'][$key]['otherDeliveryAddress'] = @$quuery->result_array()[0];
			...
			$todo['tickets'][$key]['paymentDetails'] = @$quuery->result_array()[0];

			$this->db->select("*")->from("tb_mp_payments");			
			$quuery                     = $this->db->where("idPayment = " , @$ticket['idPaymentDeliveryKf'])->get();
			$todo['tickets'][$key]['paymentDeliveryDetails'] = @$quuery->result_array()[0];
			...
			$this->db->join('tb_type_attendant', 'tb_type_attendant.idTyepeAttendant = tb_user.idTyepeAttendantKf', 'left');
			$quuery                        = $this->db->where("idTicketKf = " , @$ticket['idTicket'])->get();
			$todo['tickets'][$key]['changes_history'] = @$quuery->result_array();
			if (@$ticket['isBillingUploaded']==1){
				$this->db->select("*")->from("tb_ticket_files_list");
				$quuery                        = $this->db->where("idTicketKf = " , $ticket['idTicket'])->get();
				$todo['tickets'][$key]['billingReceiptDetails'] = @$quuery->result_array();
			}else{
				$todo['tickets'][$key]['billingReceiptDetails'] = null;
			}
			$this->db->select("*")->from("tb_ticket_delivery_company");
			$quuery                     = $this->db->where("idDeliveryCompany = " , @$ticket['idDeliveryCompanyKf'])->get();
			$todo['tickets'][$key]['deliveryCompanyDetails'] = @$quuery->result_array()[0];
			...
			$quuery                            = $this->db->where("idTicketKf = " , @$ticket['idTicket'])->get();
			if ($quuery->num_rows() > 0) {
				$todo['tickets'][$key]['refundsDetails'] = @$quuery->result_array();
			}else{
				$todo['tickets'][$key]['refundsDetails'] = null;
			}
			...
			$todo['tickets'][$key]['building']['isInitialDeliveryActive'] = $rs9;
			$todo['tickets'][$key]['building']['initial_delivery'] = $rs9;
			if($quuery->num_rows()>0){
				//print_r($rs['customers'][$i]['initial_delivery'][0]['expirationDate']);
				$dateString = $todo['tickets'][$key]['building']['isInitialDeliveryActive'][0]['expirationDate'];
				$dateString = $todo['tickets'][$key]['building']['initial_delivery'][0]['expirationDate'];
				...
				if ($currentDateFormatted > $dateToCompareFormatted) {
					$todo['tickets'][$key]['building']['isInitialDeliveryActive'][0]['expiration_state'] = true;
					$todo['tickets'][$key]['building']['initial_delivery'][0]['expiration_state'] = true;
				}else{
					$todo['tickets'][$key]['building']['isInitialDeliveryActive'][0]['expiration_state'] = false;
					$todo['tickets'][$key]['building']['initial_delivery'][0]['expiration_state'] = false;
				}
			}
		}
