        ...
        switch (opt){
            ...
            case "addNewKeyManual":
                ...
                if (!$scope.tmpKey.new.autoComplete){
                  ...
                  console.log("$scope.rsNewKeychainList length:", $scope.rsNewKeychainList.length);
                  console.log("$scope.rsExistingKeyList length:", $scope.rsExistingKeyList.length);
                  console.log("$scope.tkupdate.keys length:", $scope.tkupdate.keys.length);
                  ...
                  // helper: verificar existencia local
                  ...
                  (async () => {
                    while (added < (totalNeeded - $scope.rsNewKeychainList.length)) {
                      let newCode = (baseCode + ai).toString();
                      let newCodeExt = (baseCodeExt + ai).toString();
                      ai++;
                      ...
                      // validar remotamente (esperar respuesta)
                      const existsRemotely = await $scope.findKeyByCodeFn(newCode, $scope.tkupdate.building.idClient);
                      const codeExistsInBuilding = (existsRemotely === 1);

                      if (codeExistsInBuilding) {
                        console.log("Código ya existe en edificio:", newCode);
                        continue;
                      }

                      // si llegamos aquí → código válido, lo agregamos
                      console.log("AGREGANDO CÓDIGO NUEVO:", newCode);
